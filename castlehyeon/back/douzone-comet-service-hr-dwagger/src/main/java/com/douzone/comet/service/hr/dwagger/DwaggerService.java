...
public class DwaggerService extends DzCometService {
	...
	public List<DwaggerModel> dwagger_totalList(
			...
			@DzParam(key = "condition_cd", desc = "검색조건코드", paramType = DzParamType.QueryString) String condition_cd)
			throws Exception {
		APIDataCall callDatas = new APIDataCall();
		List<DwaggerModel> dwaggerModelList = new ArrayList<DwaggerModel>();
		// XSS 공격에 대응하기 위해 HTMLUtils를 사용한다.
		// 사용자에게 직접 입력받는 키워드는 아래의 코드를 권장한다.
		String escapedKeywords = HtmlUtils.htmlEscape(keywords);
		try {
			System.out.println("Service cacheModels===>" + cacheModels);
			String result = callDatas.callApi(escapedKeywords, condition_cd);
			System.out.println("callApi결과 result ===>" + result);
			// 어째서인지 자바 객체를 굳이 JSON으로 직렬화하지 않아도 올바르게 데이터가 전송된다. 아마 DzApi가 RestController로
			// 작동하기 때문일 듯.
			// 결과값을 찾을 수 있다면
