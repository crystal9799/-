...
public class HRPamprg00100_X10005Service extends DzCometService {
	...
	@Transactional(rollbackFor = Exception.class)
	@DzApi(url = "/excelUpload_HR_URGDBASETBL_INFO_X10005MST", desc = "엑셀 업로드", httpMethod = DzRequestMethod.POST)
	public boolean pamodm01550x10005_excel_upload(
			@DzParam(key = "BIZAREA_CD", desc = "사업장정보", paramType=DzParamType.Body) List<Pamprg00100_X10005Model> BIZAREA_CD,
			@DzParam(key = "excelFormGridDataItems", desc = "저장데이터", paramType=DzParamType.Body) String excelFormGridDataItems 
			) throws Exception {
		try {
...
//				for(Pamprg00100_X10005Model row : saveData) { // 데이터 저장 
//					
//					row.setCompany_cd(this.getCompanyCode());
//					row.setHwrt_insr_yn("Y");
//					
//					//데이터검증
//					String MSG = checkOverTimeDTL(row);
//					
//					
//					if(!MSG.equals("OK")) {
...
//			for(Pamprg00100_X10005Model row : saveData) {
//				row.setCompany_cd(this.getCompanyCode());
//				row.setHwrt_insr_yn("Y");
//				row.setInsert_id(userId);
//				row.setUpdate_id(userId);
//				
//				//데이터중복 체크
//				Integer dupData = (Integer) dao.selectObject(className + "check_over_dtl", row);
//				
//				//증복된 데이터이면 업데이트
//				if(dupData > 0) {
//					dao.update_over_dtl(className + "update_over_dtl", row);
//				} else {
//					dao.insert_over_dtl(className + "insert_over_dtl", row);
//				}
//			}
